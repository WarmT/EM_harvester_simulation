...
def main():
    ...
    for i, fill in enumerate(fillratio):
        toolbox.unregister("evaluate")
        toolbox.register("evaluate", evalOneMax, r_o=r_o, h=h, gap=gap, k_co=fill, d_co=d_co,
                         a=10.0, f=50.0, m_Br=m_Br, two_coils=two_coils, printing=False)
    ...
    plt.plot(fillratio, P1, label="One Coil")
    plt.plot(fillratio, P2, label="Two Coils")
    plt.ylabel(r"$P_\mathrm{load,max}\,\,[\mathrm{mW}]$", fontsize='x-large')
    plt.xlabel(r"$k_\mathrm{co}$", fontsize='x-large')
    plt.legend(loc=2)
