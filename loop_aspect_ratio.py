...
def main():
    ...
    aspect_ratios = np.linspace(0.1, 1.5, 29)
#    aspect_ratios = np.linspace(0.1, 0.9, 9)
    vol = 2 * 0.01 ** 3
    coils = [False, True]
    P1 = np.zeros(aspect_ratios.size)
    P2 = np.zeros(aspect_ratios.size)
    ...
    for two_coils in coils:
        ...
        for i, aspect in enumerate(aspect_ratios):
            if two_coils:
                print "%4.2f & %5.3f" % (aspect, P2[i])
            else:
                print "%4.2f & %5.3f" % (aspect, P1[i])

    print "\nBr = %.1f T, d_co = %d um, k_co = %.3f" % (m_Br, d_co*1e6, k_co)

    fig, axes = plt.subplots(facecolor='white', figsize=(12, 6))
    plt.plot(aspect_ratios, P2, 'r', label=r"Two coils $P_\mathrm{load}$")
    plt.plot(aspect_ratios, P1, 'b', label=r"One coil $P_\mathrm{load}$")
    plt.xlabel(r"aspect ratio", fontsize='large')
    plt.ylabel(r"$P_\mathrm{load}\,\,[\,\mathrm{mW}\,]$", fontsize='large')
    plt.legend(loc=4)
    fig.set_tight_layout(True)
    plt.show(block=False)
    plt.savefig("pics/loop_aspect_ratios.pdf")
